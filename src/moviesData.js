
    export const moviesData = [
      {
        id: 0,
        titre: "Inception",
        description: "Un voleur expérimenté entre dans les rêves des autres pour voler des secrets.",
        posterURL: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSst4d8lj72xNuuGSbj7e_UVZi2lQ86RC2FqQ&s",
        rate: 3,
        trailer: "https://www.youtube.com/embed/HcoZbHBDHQA?si=Mk_WbAPZ2ImS15Wf"
      },
      {
        id: 1,
        titre: "The Dark Knight",
        description: "Batman affronte le Joker, un criminel anarchiste semant le chaos à Gotham.",
        posterURL: "https://image.tmdb.org/t/p/w500/qJ2tW6WMUDux911r6m7haRef0WH.jpg",
        rate: 5,
        trailer: "https://www.youtube.com/embed/UMgb3hQCb08?si=3mYVWKM6SM17hqJO"
      },
      {
        id: 2,
        titre: "Interstellar",
        description: "Un groupe d'explorateurs voyage à travers un trou de ver à la recherche d'une nouvelle planète habitable.",
        posterURL: "https://image.tmdb.org/t/p/w500/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg",
        rate: 4,
        trailer: "https://www.youtube.com/embed/UDVtMYqUAyw?si=kT5pK6bRx0s_95rZ"
      },
      {
        id: 3,
        titre: "The Matrix",
        description: "Un programmeur découvre que la réalité dans laquelle il vit est une simulation.",
        posterURL: "https://image.tmdb.org/t/p/w500/f89U3ADr1oiB1s9GkdPOEpXUk5H.jpg",
        rate: 5,
        trailer: "https://www.youtube.com/embed/vKQi3bBA1y8?si=jCIB71snM5eat2js"
      },
      {
        id: 4,
        titre: "Fight Club",
        description: "Un employé de bureau insomniaque crée un club de combat avec un vendeur de savon charismatique.",
        posterURL: "https://image.tmdb.org/t/p/w500/pB8BM7pdSp6B6Ih7QZ4DrQ3PmJK.jpg",
        rate: 5,
        trailer: "https://www.youtube.com/watch?v=SUXWAEX2jlg"
      },
      {
        id: 5,
        titre: "The Shawshank Redemption",
        description: "Un homme innocent condamné à perpétuité trouve l'espoir et l'amitié en prison.",
        posterURL: "https://image.tmdb.org/t/p/w500/q6y0Go1tsGEsmtFryDOJo3dEmqu.jpg",
        rate: 4,
        trailer: "https://www.youtube.com/watch?v=6hB3S9bIaco"
      },
      {
        id: 6,
        titre: "The Godfather",
        description: "L'histoire de la famille mafieuse Corleone et de son patriarche Vito Corleone.",
        posterURL: "https://image.tmdb.org/t/p/w500/3bhkrj58Vtu7enYsRolD1fZdja1.jpg",
        rate: 3,
        trailer: "https://www.youtube.com/watch?v=sY1S34973zA"
      },
      {
        id: 7,
        titre: "Forrest Gump",
        description: "L'histoire touchante d'un homme simple qui traverse les grandes époques de l'histoire américaine.",
        posterURL: "https://image.tmdb.org/t/p/w500/h5J4W4veyxMXDMjeNxZI46TsHOb.jpg",
        rate: 5,
        trailer: "https://www.youtube.com/watch?v=bLvqoHBptjg"
      },
      {
        id: 8,
        titre: "The Lord of the Rings: The Return of the King",
        description: "La bataille finale pour la Terre du Milieu et la quête ultime de Frodon pour détruire l'Anneau Unique.",
        posterURL: "https://image.tmdb.org/t/p/w500/rCzpDGLbOoPwLjy3OAm5NUPOTrC.jpg",
        rate: 5,
        trailer: "https://www.youtube.com/watch?v=r5X-hFf6Bwo"
      },
      {
        id: 9,
        titre: "The Avengers",
        description: "Les super-héros de Marvel s'unissent pour combattre une menace mondiale.",
        posterURL: "https://image.tmdb.org/t/p/w500/RYMX2wcKCBAr24UyPD7xwmjaTn.jpg",
        rate: 4,
        trailer: "https://www.youtube.com/watch?v=eOrNdBpGMv8"
      },
      {
        id: 10,
        titre: "Gladiator",
        description: "Un général romain déchu cherche à se venger de l'empereur corrompu qui a tué sa famille.",
        posterURL: "https://image.tmdb.org/t/p/w500/ty8TGRuvJLPUmAR1H1nRIsgwvim.jpg",
        rate: 5,
        trailer: "https://www.youtube.com/watch?v=owK1qxDselE"
      },
      {
        id: 11,
        titre: "Joker",
        description: "L'origine sombre du personnage emblématique du Joker.",
        posterURL: "https://image.tmdb.org/t/p/w500/udDclJoHjfjb8Ekgsd4FDteOkCU.jpg",
        rate: 4,
        trailer: "https://www.youtube.com/watch?v=zAGVQLHvwOY"
      }
  ];
  
  
